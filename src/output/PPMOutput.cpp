...
void PPMOutput::writePixel(int, int, const Math::Vector3D &color) {
    int r = static_cast<int>(std::clamp(color.x, 0.0, 1.0) * 255);
    int g = static_cast<int>(std::clamp(color.y, 0.0, 1.0) * 255);
    int b = static_cast<int>(std::clamp(color.z, 0.0, 1.0) * 255);
    _out << r << " " << g << " " << b << "\n";
}
